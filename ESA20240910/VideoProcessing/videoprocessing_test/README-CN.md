#### 项目简介
该项目是在阿里云产品ESA（版本：2024-09-10）服务的 VideoProcessing 资源下「创建、更新和删除视频处理配置」的完整工程示例。此示例向您展示如何：
- 创建视频处理配置。
- 更新视频处理配置。
- 删除视频处理配置。
...
- **依赖关系**：
  - 资源【VideoProcessing】依赖于资源【Site】，因此在创建视频处理配置之前必须先成功创建站点。
  - 资源【Site】依赖于资源【RatePlanInstance】，因此在创建站点之前需要先成功购买套餐。

- **异步操作**：
  - 新购套餐和创建站点均为异步操作，需等待其状态更新为指定状态后再进行下一步操作。
  - 创建、更新和删除视频处理配置为同步操作，但需要确保前置资源已成功创建。
...
4. **创建视频处理配置**
   - 在站点创建成功后，调用 [CreateVideoProcessing](https://api.aliyun.com/api/ESA/2024-09-10/CreateVideoProcessing) 接口创建一个视频处理配置。

5. **更新视频处理配置**
   - 调用 [UpdateVideoProcessing](https://api.aliyun.com/api/ESA/2024-09-10/UpdateVideoProcessing) 接口更新已创建的视频处理配置。

6. **删除视频处理配置**
   - 最后，调用 [DeleteVideoProcessing](https://api.aliyun.com/api/ESA/2024-09-10/DeleteVideoProcessing) 接口删除之前创建的视频处理配置，完成整个生命周期管理。
