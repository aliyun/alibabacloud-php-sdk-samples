#### 项目简介
该项目是在阿里云产品ESA（版本：2024-09-10）服务的 ScheduledPreloadJob 资源下「创建、删除预加载任务」的完整工程示例。此示例向您展示如何：
- 创建和删除预加载任务。
- 管理依赖资源，包括套餐和站点。
...
- **依赖关系**：
  - 资源【预加载任务】依赖于资源【站点】，因此在创建预加载任务之前必须先成功创建站点。
...
4. **创建预加载任务**
   - 在站点创建成功后，调用 [CreateScheduledPreloadJob](https://api.aliyun.com/api/ESA/2024-09-10/CreateScheduledPreloadJob) 接口创建一个新的预加载任务。

5. **删除预加载任务**
   - 最后，调用 [DeleteScheduledPreloadJob](https://api.aliyun.com/api/ESA/2024-09-10/DeleteScheduledPreloadJob) 接口删除之前创建的预加载任务，完成整个生命周期管理。
